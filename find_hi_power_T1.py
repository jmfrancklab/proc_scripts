...
for nodename, postproc, IR_f_slice, int_slice, M in [
    ...
    IR.ft('t2')
    IR.ift('t2')
    best_shift = IR.get_prop('acq_params')['tau_us'] / 1e6
   ...
   # fl.show();quit()################################################################
    ...
    s = d.C.mean('nScans')
    df = s.C.get_ft_prop('t2','df')
    errors = []
    all_labels = set(s.dimlabels)
    all_labels -= set(['vd', 't2'])
    ...
    collected_variance = ndshape(
        [ndshape(s)['vd'], len(error_pathway)], ['vd', "pathways"]
    ).alloc()
    for j in range(len(error_pathway)):
        s_forerror = select_pathway(s, error_pathway[j])
        if j == 0:
            N2 = ndshape(s_forerror)['t2']
        s_forerror -= s_forerror.C.mean_all_but(['vd'])
        s_forerror.run(lambda x: abs(x) ** 2 / 2).mean_all_but(['vd'])
        s_forerror *= df ** 2  # Δf
        s_forerror *= N2
    s = select_pathway(s, IR_signal_pathway)
    s_int = s.integrate('t2').set_error(sqrt(s_forerror.data))
    #}}}
    #{{{Fitting Routine
    ...
    f.set_guess(
            M_0 = dict(value=-M+1, min = -M, max = 0),
            M_inf = dict(value=M-1, min = 0, max=M),
            R_1 = dict(value=1.0, min=0.001, max=100)
            )
    fl.next('IR fit')
    fl.plot(IR_data,'o',label='data')
    f.settoguess()
    guess = f.eval(100)
    f.fit()
    T1 = 1./f.output('R_1')
