...
print('T1(p) fit: $T_{1}(p)=%0.6fp+%0.6f$'%(m,b))
#{ Plot all T1's together
figure(figsize=(7,5))
title('$T_{1}$(p) for %s'%(' '.join(thisfile.split('_')[1:-1])))
plot(T1_vals,color='xkcd:dark magenta',ls='-',marker='o')
plt.ylabel('$T_{1}$ (s)')
plt.xlabel('power (W)')
plt.text(1,powers[-2],'T1 = %0.4fp + %0.4f'%(m,b))
plt.savefig('%s_T1s.png'%('_'.join(thisfile.split('_')[:-1])),overwrite=True,bbox_inches='tight')
plt.show()
plt.close
#}
