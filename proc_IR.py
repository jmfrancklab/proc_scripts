...
power_vals = np.r_[1.58,2.51,3.16,3.98] #1.00,0.0
# }}}

T1_values = nddata(np.zeros((len(power_vals))),'power').labels('power',power_vals)
IR_datasets = nddata(np.zeros((len(power_vals),12),dtype='complex128'),['power','vd']) # 12 or number of vd's used in run_IR
IR_datasets.labels('power',power_vals)
IR_fits =  nddata(np.zeros((len(power_vals),100),dtype='complex128'),['power','vd'])
IR_fits.labels('power',power_vals)
...
#        ['210518_F195R1a_pR_DHPC_FIR_0dBm','odnp','signal','spincore_IR_v1',
#         (-500,500),(None,75e-3),False,False,3.5,False],
        ['210518_F195R1a_pR_DHPC_FIR_30dBm','odnp','signal','spincore_IR_v1',
         (-500,500),(None,75e-3),False,False,3.5,False],
        ['210518_F195R1a_pR_DHPC_FIR_32dBm','odnp','signal','spincore_IR_v1',
         (-800,800),(None,75e-3),False,False,3.5,False],
        ['210518_F195R1a_pR_DHPC_FIR_34dBm','odnp','signal','spincore_IR_v1',
         (-500,500),(None,75e-3),False,False,3.5,False],
        ['210518_F195R1a_pR_DHPC_FIR_35dBm','odnp','signal','spincore_IR_v1',
         (-500,500),(None,75e-3),False,False,3.5,False],
        ['210518_F195R1a_pR_DHPC_FIR_36dBm','odnp','signal','spincore_IR_v1',
         (-500,500),(None,75e-3),False,False,3.5,False],
         ]):     
...
#        (-100,100), (None,50e-3), False, False, 3.5, False]]):    
    ...
    plt.text(0.75, 0.25, f.latex(), transform=plt.gca().transAxes,size='medium',
            ...
            position=(0.5,0.95),fontweight='bold')
    plt.text(0.5,0.5,'$T_{1}=$%0.3f s'%T1,transform=plt.gca().transAxes,size='medium',
            horizontalalignment='right',verticalalignment='bottom',color='k')
            #position=(1.115,0.8))#(0.9,0.05))
    plt.legend(bbox_to_anchor=(1,1.01),loc='upper left')
    #ax = plt.gca()
   
    #logger.info(strm("YOUR T1 IS:",T1))
    print("YOUR T1 IS:",T1)
    T1_values['power',i] = T1
    IR_datasets['power',i] = s_signal
    IR_fits['power',i] = f.eval(100)
#    fl.show()
...
plt.title('$T_{1}$ vs Power',fontsize='x-large')
plt.plot(power_vals,T1_values.data,'o-')
plt.xlabel('p (W)'); plt.ylabel('$T_{1} (s)$')
# In[]
#files = ['210513_F195R1a_pR_DDM_IR_0dBm',
#         '210513_F195R1a_pR_DDM_IR_30dBm',
#         '210513_F195R1a_pR_DDM_IR_31dBm',
#         '210513_F195R1a_pR_DDM_IR_32dBm',
#         '210513_F195R1a_pR_DDM_IR_33dBm',
#         '210513_F195R1a_pR_DDM_IR_34dBm']
files = ['210514_S175R1a_pR_DHPC_IR_0dBm.h5',
         '210514_S175R1a_pR_DHPC_IR_26dBm.h5',
         '210514_S175R1a_pR_DHPC_IR_28dBm.h5',
         '210514_S175R1a_pR_DHPC_IR_30dBm.h5',
         '210514_S175R1a_pR_DHPC_IR_32dBm.h5',
         '210514_S175R1a_pR_DHPC_IR_34dBm.h5']
power_vals = r_[0,30,31,32,33,34]
exp_types = ['odnp']*len(files)
nodenames = ['signal']*len(files)
postprocs = ['spincore_IR_v1']*len(files)
f_ranges = [(-200,200),
            (-200,200),
            (-200,200),
            (-200,200),
...
t_ranges = [(None,50e-3),
            ...
            (None,50e-3),
            (None,50e-3)] # s
IR_bools = [False]*len(files)
ILT_bools = [False]*len(files)
W_vals = [6,3.5,3.5,3.5,3.5,3.5] # s
plot_bools = [False]*len(files)
T1_vals = nddata(np.zeros((len(files))),'power dBm').labels('power dBm',power_vals)

for i,(thisfile,exp_type,nodename,postproc,f_range,t_range,
       IR,ILT,W,plot_all) in enumerate(zip(
               files,exp_types,nodenames,postprocs,f_ranges,
               t_ranges,IR_bools,ILT_bools,W_vals,plot_bools
               )):
    fl.basename = '_'.join(thisfile.split('_'))
    
    fl.next('spacer');plt.text(0,0,'_'.join(thisfile.split('_')),
            horizontalalignment='center',verticalalignment='center',
            position=(0.5,0.5),size='x-large')
...
#    fl.plot(f.eval(100),label='fit')
#    fl.plot(0,s_signal.data.max()+0.25*max(abs(s_signal.data)),alpha=0)
#    plt.text(0.5,0.5, f.latex(), transform=plt.gca().transAxes,size='medium',
#            horizontalalignment='center',verticalalignment='center',color='k',
#            position=(0.5,0.95),fontweight='bold')
#    plt.text(0.5,0.5,'$T_{1}=$%0.3f s'%T1,transform=plt.gca().transAxes,size='medium',
#            horizontalalignment='center',verticalalignment='center',color='k',
...
#    
## In[]
#files = ['210513_S175R1a_pR_DDM_IR_0dBm.h5',
#         '210513_S175R1a_pR_DDM_IR_27dBm.h5',
#         '210513_S175R1a_pR_DDM_IR_30dBm.h5',
#         '210513_S175R1a_pR_DDM_IR_32dBm.h5',
#         '210513_S175R1a_pR_DDM_IR_34dBm.h5']
#files = ['MISSING 0dBm data from NMR computer',
#          #/c/apps-su/spincore_apps/spincore_pp/'
#         'MISSING 26dBm data too' 
#         '210514_F195R1a_pR_DHPC_IR_28dBm.h5',
#         '210514_F195R1a_pR_DHPC_IR_30dBm.h5',
#         '210514_F195R1a_pR_DHPC_IR_32dBm.h5',
#         '210514_F195R1a_pR_DHPC_IR_34dBm.h5']
#files = ['210514_S175R1a_pR_DHPC_IR_0dBm.h5',
#         '210514_S175R1a_pR_DHPC_IR_26dBm.h5',
#         '210514_S175R1a_pR_DHPC_IR_28dBm.h5',
#         '210514_S175R1a_pR_DHPC_IR_30dBm.h5',
