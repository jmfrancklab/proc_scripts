    ...
    for searchstr, exp_type, nodename, postproc, label_str, slice_f in [
        ...
        if len(ph0.dimlabels) > 0:
            assert len(ph0.dimlabels) == 1, (
                repr(psd.ndshape(ph0.dimlabels)) + " has too many dimensions"
            )
            ph0 = prscr.zeroth_order_ph(ph0, fl=fl)
            psd.logger.info(psd.strm("phasing dimension as one"))
        else:
            psd.logger.info(
                psd.strm(
