...
def fake_data(
    ...
    indirect_size = np.prod([ndshape(clean_data)[j] for j in axis_coords.keys() if j != direct])
    ...
    fl.plot(frq_noise)
    frq_noise.ift('temp')
    frq_noise /= np.sqrt(ndshape(frq_noise)['temp']) * frq_noise.get_ft_prop('temp', 'df') # normalization
