...
class calib_info(object):
    ...
    def use_calibration(self, calibration_name):
        if calibration_name != self.current_calib_name:
            ...
            assert (
                len(calibration_name) > 0
            ), "you MUST use a calibration name and set the values `[calibration_name] q` and `[calibration_name] propFactor` in your _pyspecdata file!"
            ...
            try:
                ...
                raise RuntimeError(
                    f"I expect a line in the [General] block of your pyspecdata config file (in your home directory) that sets the value of the following variables:\n{calibration_name} q\n{calibration_name} propFactor\n\n(I can't run without these)"
                )
    ...
    def use_diameter(self, diameter_name):
        if diameter_name != self.current_diam_name:
            ...
            assert (
                len(diameter_name) > 0
            ), "you MUST use a diameter name and set the values `[diameter_name] q` and `[diameter_name] propFactor` in your _pyspecdata file!"
            try:
                self.d = float(pyspec_config.get_setting(f"{diameter_name} diameter"))
            except Exception:
                raise RuntimeError(
                    f"(note this the second in a similar pair of errors) I expect a line in the [General] block of your pyspecdata config file (in your home directory) that sets the value of the following variables:\n{diameter_name} diameter"
                )
...
def QESR_scalefactor(d, calibration_name=None, diameter_name=None):
    ...
    c = Q_(1, "dimensionless")  # the first fraction on eq 2-17 -- in bruker E500 manual
