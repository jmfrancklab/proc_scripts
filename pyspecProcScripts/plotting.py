...
class fl_mod(figlist_var):
    ...
    def side_by_side(self, plotname, s, thisrange):
        ...
        if "power" in s.dimlabels:
            self.image(
                forplot.C.setaxis("power", "#").set_units("power", "scan #")
            )
        else:
            self.image(forplot)
...
def expand_limits(thisrange, s, axis="t2"):
    ...
    retval = np.array(
        [
            thisrange[j] if thisrange[j] is not None else full_range[j]
            for j in [0, -1]
        ]
    )
    m = np.mean(retval)
    ...
    return tuple(
        full_range[j]
        if retval[j] * sgn[j] > full_range[j] * sgn[j]
        else retval[j]
        for j in range(2)
    )
...
def draw_limits(thisrange, s):
    ...
    pairs = [
        [thisrange[j], full_range[j] + 0.5 * dw * sgn[j]] for j in range(2)
    ]
