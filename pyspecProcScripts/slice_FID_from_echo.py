...
def slice_FID_from_echo(
    s, max_t=None, signal_pathway={"ph1": 1, "ph2": -2}, fl=None
):
    ...
    if len(coh_slice.dimlabels) > 0:
        assert len(coh_slice.dimlabels) == 1, (
            repr(ndshape(coh_slice.dimlabels)) + " has too many dimensions"
        )
    ...
    if fl is not None:
        fl.side_by_side(
            "time domain (after filtering and phasing)\n$\\rightarrow$ use to adjust time range",
            s,
            (0, max_t),
        )
    s = s["t2":(0, max_t)]
