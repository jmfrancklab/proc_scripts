...
def rough_table_of_integrals(
    ...
    if signal_range is None:
        center_of_slice, half_slice = find_peakrange(
        ...
        half_slice = 0.5 * np.diff(signal_range).item()
    signal_range_expanded = (
        center_of_slice + expansion * r_[-1, 1] * half_slice
    )
    assert fl is not None, "for now, fl can't be None"
    # {{{ set up subplots
    fig, ((ax1, ax2), (ax3, ax_last)) = plt.subplots(2, 2)
    fig.set_figwidth(15)
    fig.set_figheight(6)
    fig.suptitle(title)
