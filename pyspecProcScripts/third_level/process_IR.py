...
def process_IR(
    ...
    if ILT:
        ...
        if plot_Lcurve:
            ...
            def vec_lcurve(l):
                return s.C.nnls(
                    "vd", T1, lambda x, y: 1.0 - 2 * np.exp(-x / y), l=l
                )
        ...
        s.setaxis("t2", lambda x: x / (sfo1)).set_units("t2", "ppm")
        s.set_prop("x_inverted", True)
        soln = s.real.C.nnls(
            "vd", T1, lambda x, y: 1.0 - 2.0 * np.exp(-x / y), l=this_l
        )
