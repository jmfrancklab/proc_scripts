...
def process_IR(s, this_l = 0.032,
    ...
    if flip:
        s *= -1
    #{ Sam's simple integral:
    simple_integral = False
    if simple_integral:
        s_int  = s['ph1',0]['ph2',1].C
        s_int.integrate('t2')
    ...
    if fl is not None:
        if simple_integral:
            ...
            fl.plot(s_int,'o',markersize=6,label='data')
            fl.plot(s_int.imag,'o',markersize=6,label='imaginary')
            fl.plot(f.eval(100),'-',label='fit')
            plt.text(0.5,-0.05,f.latex(),transform=plt.gca().transAxes,size='medium',ha='center',va='bottom',color='k',position=(0.5,-0.05),fontweight='bold')
