...
def hermitian_function_test(
    ...
    echo_before         int    
        Upper limit of the echo time usually pulled from the acquisition 
        parameters.
    basename:           str
        String for figure titles
    show_zoom:          boolean
        option to show zoomed in echo - optimal for shorter tau values

    """
    # {{{ define the original bounds and zero fill
    ...
    logging.debug("trying to extend from",s_ext.getaxis(direct)[r_[0,-1]],"to",
    ...
    if fl is not None:
        ...
        if show_zoom:
            ...
            fl.plot(forplot[direct:(0.1*orig_bounds)], label="first energy term",
                    human_units=False)
    # }}}
    # {{{ calculate the correlation between the echo and its hermitian
    #     conjugate
