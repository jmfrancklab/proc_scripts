    ...
    def decorate_axes(idx, remaining_dim, depth):
        ...
        for j in range(a_shape[thisdim]):
            ...
            if my_data.get_ft_prop(thisdim) == True:
                ...
                if thisdim == 'ph1':
                    if j == 0:
                        draw_span(last_axes, first_axes, "0", this_label_num = depth)
                    elif j == 1:   
                        draw_span(last_axes, first_axes, "0.25", this_label_num=depth)
                    elif j == 2:   
                        draw_span(last_axes, first_axes, "0.5", this_label_num=depth)
                    elif j == 3:   
                        draw_span(last_axes, first_axes, "0.75", this_label_num=depth)
                if thisdim == 'ph2':
                    if j == 0:
                        draw_span(last_axes, first_axes, "0", this_label_num = depth)
                    elif j == 1:   
