...
init_logging("debug")
d = find_file(
    "201209_sqwv_sol_probe_1", exp_type="ODNP_NMR_comp/test_equipment", expno="capture1"
)
#s = find_file(
...
class fl_ext(figlist_var):
    ...
    def abs_re_plot(fl, d, add_text="", show_angle=True):
...
with fl_ext() as fl:
    ...
    #tukey_filter_s = s.fromaxis("t")["t":tuple(frq_range)].run(lambda x:tukey(len(x)))
    d["t":tuple(frq_range)] *= tukey_filter
    #s["t":tuple(frq_range)] *= tukey_filter_s
    ...
    #    fl.plot(abs(forplot_s)['ch':j],alpha=0.5,plottype='semilogy',label=f"CH{j} orig")
    #    fl.plot(abs(s)['ch':j][lambda x: abs(x) > 1e-10], alpha=0.5, plottype="semilogy",label=f"CH{j} filtered")
    #fl.grid     
