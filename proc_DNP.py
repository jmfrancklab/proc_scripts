...
# {{{process enhancement
for nodename, postproc in [("enhancement", "spincore_ODNP_v1")]:
    d = find_file(
        thisfile,
        exp_type=exp_type,
        expno=nodename,
    ...
    dsign = determine_sign(
        select_pathway(dslice, E_signal_pathway, mult_ph_dims=False)
    )
...
ksigs_T = (
    (0.0015167 / C)
    * (1 - enhancement["power", : idx_maxpower + 1])
    * (R1p_fine)
)
