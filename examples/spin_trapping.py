...
In this example, we perform quantitative ESR by fitting a sample with spin trapping.
...
def D(n, d):
    ...
    h_m *= mod_calib
    arg = abs(h_m * u / 2)
    # equation below gives h_m/4, but I have already divided by h_m during scaling, so this should be divided by h_m as well
    # I also do not divide by 4 here, because that would mess up my scaling
...
# numbers of the hyperfine coupled H and N (NOTE: for TEMPOL, there is no H, only N).
# 
...
d.fit(use_jacobian=False)
forplot = d.eval()
psd.plot(forplot["B":zoom_tuple], label="vary splitting, as well", alpha=0.7, ax=ax)
# }}}
d.guess_parameters = d.fit_parameters
...
d.fit(use_jacobian=False)
forplot = d.eval()
psd.plot(forplot["B":zoom_tuple], label="vary amplitude, as well", alpha=0.7, ax=ax)
# }}}
d.guess_parameters = d.fit_parameters
# {{{ vary everything
for name, par in d.guess_parameters.items():
