...
c_re, c_im, A, ll, nu, nu0 = sp.symbols('c_re c_im A lambda_L nu nu_0', real=True)
    ...
    for filename, thislabel in [
        ...
        for j in range(3):
            if j == 0:
                ...
                zoom_data_nd.functional_form = abs(c_re + 1j*c_im - A / (1+1j*2*(nu-nu0)/ll))
                zoom_data_nd.set_guess(
                        ...
                        nu_0=0)
                zoom_data_nd.fit()
                fl.plot(zoom_data_nd, 'o', label=thislabel+f": width={zoom_data_nd.output()['lambda_L']:0.5g}", alpha=0.5)
