...
with psd.figlist_var() as fl:
    for searchstr, exptype, nodename, label, axs in [
        ...
        d_raw = s.C
        fl.image(
            pscr.select_pathway(s["t2":(-500, 500)].C, signal_pathway),
            ax=ax1[axs[0]][axs[1]],
        )
        d_unc = pscr.select_pathway(
            s["t2":signal_range].C.real, s.get_prop("coherence_pathway")
        ).integrate("t2")
        # }}}
        ...
        d0 = pscr.select_pathway(
            s["t2":signal_range].real.C, s.get_prop("coherence_pathway")
        ).integrate("t2")
        ...
        d_raw *= mysign
        fl.image(
            pscr.select_pathway(d_raw, s.get_prop("coherence_pathway")),
            ax=ax2[axs[0]][axs[1]],
        )
        s = d_raw
        s.ift("t2")
        ...
        s /= pscr.zeroth_order_ph(
            pscr.select_pathway(s["t2":0.0], signal_pathway)
        )
        s = s["t2":(0, None)]
        s *= 2
        s["t2", 0] *= 0.5
        ...
        fl.image(
            pscr.select_pathway(s, s.get_prop("coherence_pathway")),
            ax=ax3[axs[0]][axs[1]],
        )
        ...
        fl.plot(fin, "o", ax=ax4[axs[0]][axs[1]])
        ...
        f.set_guess(
            A=dict(
                value=fin.data.max() * 1.2,
                min=fin.data.max() * 0.1,
                max=fin.data.max() * 1.2,
            ),
        ...
        f.fit()
        fit = f.eval()
        fl.plot(fit, ax=ax4[axs[0]][axs[1]])
