...
with psd.figlist_var() as fl:
    for searchstr, exptype, nodename, fl.basename in [
        ...
        if "nScans" in s.dimlabels:
            fl.image(pscr.select_pathway(s.C.mean("nScans"), signal_pathway), ax=ax_list[0])
        else:
            fl.image(pscr.select_pathway(s.C, signal_pathway), ax=ax_list[0])
        ...
        for j in range(5):
            corr = clock_correct(
                ...
                * mysgn
                * np.exp(-1j * 2 * np.pi * total_corr * s.fromaxis("beta")),
                "beta",
            )
            total_corr += corr
        s *= np.exp(-1j * 2 * np.pi * total_corr * s.fromaxis("beta"))
        mysigns = pscr.determine_sign(
            pscr.select_pathway(s["t2":signal_range], s.get_prop("coherence_pathway"))
        )
        ...
        for j in range(len(s.getaxis("beta"))):
            ...
            s["beta", j] /= ph0
        fl.image(pscr.select_pathway(s, s.get_prop("coherence_pathway")), ax=ax_list[1])
        ...
        s.ft("t2")
        s *= -mysigns
        fl.image(pscr.select_pathway(s, s.get_prop("coherence_pathway")), ax=ax_list[2])
        ...
        if "nScans" in s.dimlabels:
            s.mean("nScans")
        fl.plot(
            pscr.select_pathway(s["t2":signal_range], signal_pathway).C.real.integrate("t2"),
            "o",
            label=fl.basename,
            human_units=False,
        )
