...
error_pathway = (
    set(((j, k) for j in range(2) for k in range(4)))
...
std_off_pathway = data["ph1", 0]["ph2", 0].C.run(lambda x: abs(x) ** 2).mean_all_but(
    "t2"
).mean("t2").run(sqrt) / sqrt(2)
logger.debug(strm("check the std after FT -- manually", std_off_pathway))
logger.debug(
    strm(
        "check the std after FT -- manually don't collapse vd",
        data["ph1", 0]["ph2", 0]
...
for bounds in [
    manual_slice,  # leave this as a loop so user can experiment with different bounds
]:
    ...
    fl.plot(s_int, '.', capsize=6,
            label='std calculated from integral_w_error')
    logger.debug(
...
fl.plot(s_int,'.',capsize=6,label='propagate variance from integral of inactive pathway',alpha=0.5)
fl.show()
