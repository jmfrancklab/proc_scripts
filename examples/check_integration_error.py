...
for j in range(n_repeats):
    ...
    print('#%d'%j)
std_off_pathway = data['ph1',0]['ph2',0]['t2':bounds].C.run(lambda x: abs(x)**2).mean_all_but(['t2','vd']).mean('t2').run(sqrt)
print("off-pathway std",std_off_pathway/sqrt(2),"programmed std",fake_data_noise_std)
