...
with psd.figlist_var() as fl:
    ...
    log_vs_time = psd.nddata(
        ...
        log_array[
            "time"
        ],  # to the "time" field of the structured array that comes from the log
    )
    fl.next("power log")
    ...
    s /= prscr.zeroth_order_ph(
        prscr.select_pathway(s["t2":0], Ep_signal_pathway)
    )
    s_align = s.C
    FID = s.C
    FID = FID["t2":(0, None)]
    ...
    fl.next("test time")
    fl.text(
        "I am making a copy of the data to apply apodization and then obtain"
        " the optimal shift from the alignment. But this apodization is not"
    ...
    myfilter = np.exp(
        -abs(
            (
                s_align.fromaxis("t2")
                - s_align.get_prop("acq_params")["tau_us"] * 1e-6
            )
        )
        / filter_timeconst
    )
    ...
    if "nScans" in s.dimlabels:
        mysgn = prscr.determine_sign(
            prscr.select_pathway(
                s_align[
                    "t2" : (Ep_f_slice[0] + 500, Ep_f_slice[-1] - 500)
                ].C.mean("nScans"),
                Ep_signal_pathway,
            ),
            (Ep_f_slice[0] + 500, Ep_f_slice[-1] - 500),
        )
    else:
        mysgn = prscr.determine_sign(
            prscr.select_pathway(
                s_align["t2" : (Ep_f_slice[0] + 500, Ep_f_slice[-1] - 500)],
                Ep_signal_pathway,
            ),
            (Ep_f_slice[0] + 500, Ep_f_slice[-1] - 500),
        )
    ...
    opt_shift, sigma, my_mask = prscr.correl_align(
        (s_align.C),
    ...
    for j, (time_start, time_stop) in enumerate(
        ...
        power_vs_time[powername, j] = power_axis[
            powername:((time_start), (time_stop))
        ].mean(powername, std=True)
    ...
    for j in range(thermal_scans):
        fl.plot(
            select_pathway(d["power", j].C, Ep_signal_pathway),
            label="thermal %d" % j,
        )
    mysgn = determine_sign(
        select_pathway(
            d["t2" : (Ep_f_slice[0] + 500, Ep_f_slice[-1] - 500)].C,
            Ep_signal_pathway,
        ),
        (Ep_f_slice[0] + 500, Ep_f_slice[-1] - 500),
    )
    ...
    s_int, this_frq_slice = integral_w_errors(
        d.C,
        Ep_signal_pathway,
        error_pathway,
        indirect="power",
