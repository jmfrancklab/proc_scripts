...
for expression, orderedDict, signal_pathway, indirect, label in [
        ((23*(1 - 2*s.exp(-vd / 0.2)) * s.exp(+1j*2*s.pi*100*(t2) - abs(t2)*50*s.pi)),
            ...
            'vd',
            'IR'),
        ((23*(1-(32*power/(0.25 +power))*150e-6*659.33)*s.exp(+1j*2*s.pi*100*(t2) - abs(t2)*50*s.pi)),
            ...
            best_shift,max_shift = hermitian_function_test(select_pathway(data,signal_pathway).C.mean(indirect))
