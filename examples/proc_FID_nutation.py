...
with psd.figlist_var() as fl:
    ...
    if "nScans" in s.dimlabels:
        s.mean("nScans")
    s.set_plot_color("g")  # this affects the 1D plots, but not the images, etc.
    ...
    s = psd.lmfitdata(s)
    s.functional_form = A * sp.exp(-R * beta) * sp.sin(beta / beta_ninety * sp.pi / 2)
    ...
    s.set_guess(
        ...
        R=dict(value=1e3 * prefactor_scaling, min=0, max=3e4 * prefactor_scaling),
        beta_ninety=dict(
